...
namespace Microsoft.PythonTools.Analysis.Analyzer {
    public sealed class PythonSourceStreamReader : TextReader {
        ...
        public PythonSourceStreamReader(Stream stream, bool throwOnInvalidChar, ErrorSink errors) {
            _stream = stream;
            _errors = errors;
        ...
        private async Task FillBufferAsync(int bytesRequested = 0) {
            if (bytesRequested < BufferFillChunkSize) {
        ...
        private int FindNextLine(byte[] buffer, int start, int end, out NewLineKind nlkind) {
            ...
            if (slashR >= 0) {
                if (slashN >= 0 && (slashN < slashR || slashN == slashR + 1)) {
                    nlkind = NewLineKind.CarriageReturnLineFeed;
                    return slashN + 1;
                }
                nlkind = NewLineKind.CarriageReturn;
                return slashR + 1;
            } else if (slashN >= 0) {
                nlkind = NewLineKind.LineFeed;
                return slashN + 1;
            }
            nlkind = NewLineKind.None;
            return -1;
        }
        ...
        private async Task ReadEncodingAsync() {
            ...
            await FillBufferAsync(ReadLineBufferSize * 4);
            if (_buffer == null) {
                return;
            }
            ...
            try {
                ...
                if (read < 3) {
                    // No BOM or coding comment, so assume UTF-8 (handled in finally)
                    return;
                }
                ...
                if (firstLine[0] == 0xEF && firstLine[1] == 0xBB && firstLine[2] == 0xBF) {
                    ...
                    _codingComment = "\uFEFF";
                    firstLine = firstLine.Skip(3).ToArray();
                    readIndex += 3;
                }

                var lineStarts = new List<NewLineLocation>();
                for (int lineNo = 1; lineNo <= 2; ++lineNo) {
                    NewLineKind nlkind;
                    int eol = FindNextLine(firstLine, 0, read, out nlkind);
                    if (eol < 0) {
                        eol = read;
                    }

                    lineStarts.Add(new NewLineLocation(readIndex + eol, nlkind));
                    ...
                    if (match.Success) {
                        var encoding = CodecsInfo.GetEncoding(match.Groups[1].Value, _throwOnInvalidChar);
                        string error = null;
                        if (encoding == null) {
                            error = "encoding problem: unknown encoding (line {0})".FormatUI(lineNo);
                        } else if (_encoding == null) {
                            _encoding = encoding;
                            _codingComment = text;
                        } else if (_encoding != encoding) {
                            error = "file has both Unicode marker and PEP-263 file encoding.  You must use \"utf-8\" as the encoding name when a BOM is present.";
                        }
                        ...
                        if (!string.IsNullOrEmpty(error)) {
                            _errors?.Add(
                                error,
                                lineStarts.ToArray(),
                                readIndex + match.Groups[1].Index,
                                readIndex + match.Groups[1].Index + match.Groups[1].Length,
                                ErrorCodes.SyntaxError,
                                Severity.Error
                            );
                        }
                    ...
                    readIndex += eol;
                    var secondLine = new byte[ReadLineBufferSize];
                    int read2 = _buffer.Read(secondLine, 0, secondLine.Length);
                    firstLine = firstLine.Skip(eol).Concat(secondLine.Take(read2)).ToArray();
                    read = eol + read2;
                }
            } finally {
        ...
        private async Task GetLineAsync() {
            ...
            int read = _buffer.Read(buffer, 0, buffer.Length);
            int totalRead = read;
            int eol = FindNextLine(buffer, 0, read, out nlkind);
            while (eol < 0 || eol == buffer.Length) {
                ...
                eol = FindNextLine(buffer, bufferStart, totalRead, out nlkind);
            }
