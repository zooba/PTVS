...
namespace Microsoft.PythonTools.Parsing {
    sealed class Tokenizer2 {
        ...
        public Tokenizer2(PythonLanguageVersion version) {
            _version = version;
            _lineNumber = 0;
            _lineStart = 0;
        }
        ...
        public IEnumerable<Token2> GetTokens(string line) {
            _lineNumber += 1;
            if (string.IsNullOrEmpty(line)) {
                return Enumerable.Empty<Token2>();
            }
        ...
        private IEnumerable<Token2> GetTokensWorker(string line, int lineStart, int lineNumber) {
            int c = 0;
            while (c < line.Length) {
                ...
                if (!string.IsNullOrEmpty(_multilineStringQuotes)) {
                    ...
                    yield return new Token2(TokenCategory.StringLiteral, _multilineStringStart, stringEnd);
                    yield return new Token2(TokenCategory.CloseQuote, stringEnd, _multilineStringQuotes.Length);
                ...
                var token = new Token2(category, new SourceLocation(lineStart, lineNumber, c + 1), len);
                yield return token;
                c += len;
                ...
                if (category == TokenCategory.OpenQuote) {
                    if (len >= 3) {
                        ...
                        if (c2 < c) {
                            yield return new Token2(TokenCategory.StringLiteral, token.Span.End, line.Length - c);
                            c = line.Length;
                        } else {
                            token = new Token2(TokenCategory.StringLiteral, token.Span.End, c2 - c);
                            yield return token;
                            yield return new Token2(TokenCategory.CloseQuote, token.Span.End, 1);
                            c = c2 + 1;
                        }
                    }
                }
        ...
        private void GetNextToken(string line, int start, out int length, out TokenCategory category) {
            ...
            MaybeReadOperator(line, ref end, ref category);
            if (category != TokenCategory.Error) {
                length = end - start;
                return;
            }
        ...
        private static void MaybeReadOperator(string line, ref int end, ref TokenCategory category) {
            if (end >= line.Length) {
                return;
            }

            char c = line[end];
            char c2 = (end + 1 < line.Length) ? line[end + 1] : '\0';
            char c3 = (end + 2 < line.Length) ? line[end + 2] : '\0';
            end += 1;
            switch (c) {
                // X= operators
                case '!':
        ...
        public IEnumerable<Token2> GetRemainingTokens() {
            var eof = new SourceLocation(_lineStart, _lineNumber, 1);

            if (!string.IsNullOrEmpty(_multilineStringQuotes)) {
                yield return new Token2(TokenCategory.StringLiteral, _multilineStringStart, eof);
            }

            yield return Token2.EOF;
        }
    }
}
