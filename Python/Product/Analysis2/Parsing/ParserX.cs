...
namespace Microsoft.PythonTools.Analysis.Parsing {
    ...
    public class Parser {
        ...
        // state:
        private TokenInfo _token, _eofToken;
        private readonly List<TokenInfo> _lookahead;
        ...
        private struct TokenInfo {
            public static readonly TokenInfo Empty = new TokenInfo();

            public TokenInfo(Token token, TokenKind kind = TokenKind.Unknown) {
                Token = token;
                Kind = kind;
            }
        ...
        public ParseResult ParseTopExpression() {
            ...
            ReturnStatement ret = new ReturnStatement(ParseTestListAsExpr());
            _alwaysAllowContextDependentSyntax = false;
            ret.Span = SourceSpan.None;
        ...
        public void Reset(FutureOptions languageFeatures) {
            ...
            _tokens = _tokenization.AllTokens.GetEnumerator();
            _token = TokenInfo.Empty;
            _lookahead.Clear();
            _eofToken = TokenInfo.Empty;
        ...
        private void ReportSyntaxError(
            ...
            ReportSyntaxError(t, t.Span, errorCode, allowIncomplete);
        }
        ...
        private void ReportSyntaxError(
            ...
            if (allowIncomplete &&
                (t.Category == TokenCategory.EndOfLine || t.Category == TokenCategory.EndOfStream || t.Category == TokenCategory.Comment)) {
                errorCode |= ErrorCodes.IncompleteStatement;
            }

            ReportSyntaxError(span, GetErrorMessage(t, errorCode), errorCode);
        }

        private string GetErrorMessage(Token t, int errorCode) {
            string msg;
            if ((errorCode & ~ErrorCodes.IncompleteMask) == ErrorCodes.IndentationError) {
                msg = "expected an indented block";
            } else if (t.Category != TokenCategory.EndOfStream) {
                msg = string.Format(CultureInfo.InvariantCulture, "unexpected token '{0}'", _tokenization.GetTokenText(t));
            } else {
        ...
        private void ReportSyntaxError() {
            var peek = Peek();
            ReportSyntaxError(peek.Token.Span, GetErrorMessage(peek.Token, ErrorCodes.SyntaxError));
        }

        private void ReportSyntaxError(string message) {
            var peek = Peek();
            ReportSyntaxError(peek.Token.Span, message);
        }

        internal void ReportSyntaxError(SourceSpan span, string message) {
            ReportSyntaxError(span, message, ErrorCodes.SyntaxError);
        }

        internal void ReportSyntaxError(SourceSpan span, string message, int errorCode) {
        ...
        private string GetTokenText(Token token) {
            // TODO: Caching
            return _tokenization.GetTokenText(token);
        }
        ...
        private Statement ParseStmt() {
            ...
            var p = Peek();
            while (p.Token.Category == TokenCategory.WhiteSpace) {
                attr.BeforeNode = (attr.BeforeNode ?? "") + GetTokenText(p.Token);
                Next();
                p = Peek();
            }
            if (p.Token.Category == TokenCategory.Comment) {
                attr.Comment = GetTokenText(p.Token);
                Next();
                p = Peek();
            }
            if (p.Token.Category == TokenCategory.EndOfLine || p.Token.Category == TokenCategory.EndOfStream) {
                Next();
                return new EmptyStatement { Attributes = attr };
            }
            ...
            switch (p.Kind) {
                ...
                case TokenKind.KeywordDef:
                    s = ParseFuncDef(isCoroutine: false);
                    break;
                case TokenKind.KeywordClass:
                    s = ParseClassDef();
                    break;
                case TokenKind.KeywordWith:
                    s = ParseWithStmt(isAsync: false);
                    break;
            ...
            if (s.Attributes != null) {
                s.Attributes.BeforeNode = attr.BeforeNode;
        ...
        */
        ...
        private PrintStatement ParsePrintStmt() {
            ...
            var attr = new PrintStatementAttributes {
                BeforeImage = _token.LeadingWhitespace
            };
            var start = GetStart();
            ...
            if (MaybeEat(TokenKind.RightShift)) {
                attr.BeforeRightShift = _token.LeadingWhitespace;
                attr.AfterRightShift = TakeNextWhitespace();
        ...
        private Expression ParseExpression() {
            var p = Peek();
            if (p.Token.Category == TokenCategory.WhiteSpace) {
                attr.BeforeNode = GetTokenText(p.Token);
            }
        ...
        private Expression ParseOrTest() {
            ...
            while (MaybeEat(TokenKind.KeywordOr)) {
                ...
                var start = ret.StartIndex;
                ret = new BinaryExpression(PythonOperator.Or, ret, ParseAndTest());
                if (_verbatim) {
        ...
        private Expression ParseAndTest() {
            ...
            while (MaybeEat(TokenKind.KeywordAnd)) {
                ...
                var start = ret.StartIndex;
                ret = new BinaryExpression(PythonOperator.And, ret, ParseAndTest());
                if (_verbatim) {
                    AddPrecedingWhiteSpace(ret, proceeding);
                }
                ret.SetLoc(start, GetEnd());
        ...
        */
        ...
        private SourceLocation GetEnd() {
            Debug.Assert(_token.Kind != TokenKind.Unknown, "No token fetched");
            return _token.Token.Span.End;
        }

        private SourceLocation GetStart() {
            Debug.Assert(_token.Kind != TokenKind.Unknown, "No token fetched");
            return _token.Token.Span.Start;
        }
        ...
        private TokenInfo Peek(int count = 1) {
        ...
        /// </remarks>
        private TokenInfo _FetchNext() {
            if (_eofToken.Kind != TokenKind.Unknown) {
                return _eofToken;
            }
            while (_tokens.MoveNext()) {
                var token = _tokens.Current;
                if (token.Category == TokenCategory.EndOfStream) {
                    _eofToken.Token = token;
                    _eofToken.Kind = TokenKind.EndOfFile;
                    return _eofToken;
                }

                return new TokenInfo(token, token.GetTokenKind(_tokenization));
            }
