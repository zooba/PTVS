...
# Ensure we can resolve our submodules, in case we were executed directly
orig_path = sys.path[:]
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from ptvsd.util import exec_file
from ptvsd.debugger import DONT_DEBUG, _enable_debug, _finish_debug
...
parser.add_option('-o', '--options', type='str', metavar='<options>', help='a string containing debugger-specific options')
...
parser.add_option('--connect', dest='connect', action='store_true', default=True, help='connect to a listening debugger')
...
# TODO: Refactor into more consistent code between the two attach mechanisms
if opts.connect:
    context = _enable_debug(opts.secret, (opts.interface, opts.port), opts.options)
else:
